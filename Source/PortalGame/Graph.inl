...
#include <shared/Utils/TypeUtils.h>

namespace wh {
namespace xgenaimodule {
namespace navigation {

	using wh::shared::ensure_ref;
	...
	C_Graph<NODE, EDGE>::C_Graph(bool digaph)
		...
		,	m_nextNodeIndex(0)
	{
		WH_ASSERT(digaph, "If you van to use it as nondirected graph, you must revide some code, eg. AddEdge");
	}
...
// 	}
	...
	void C_Graph<NODE, EDGE>::AddEdge(EDGE & edge)
	{
		...
		WH_ASSERT(ensure_ref(edge).From() < size && ensure_ref(edge).To() < size && ensure_ref(edge).From() >= 0 && ensure_ref(edge).To() >= 0, "Connecting nodes with wrong indexes");

		WH_UNUSED_VARIABLE(size);
...
//			}
	...
	{
		WH_ASSERT(IsNodePresent(from), "Invalid node");
	...
	{
		WH_ASSERT(IsNodePresent(index), "Invalid node");
	...
	{
		WH_ASSERT(IsNodePresent(from), "Invalid node");
		WH_ASSERT(IsNodePresent(to), "Invalid node");
	...
	{
		WH_ASSERT(false, "this is expensive, if you want to use this, have counter and keep its state when removing/adding");
